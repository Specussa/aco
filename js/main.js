...
if (circle) {
  const appCircle = () => {
    docheight.style.setProperty("--circle", `${circle.offsetWidth}px`);
  };
...
// start largepanel__product_slider
const largepanelproductSlider = document.querySelector(".largepanel__product_slider");
if (largepanelproductSlider) {
  var largepanelSlider = new Swiper(".largepanel__product_swiper", {
    loop: true,
    spaceBetween: 40,
    slidesPerView: 1,
    touchRatio: 0.2,
    ...
    speed: 1000,
    navigation: {
      nextEl: ".largepanel__product_slider_next",
      prevEl: ".largepanel__product_slider_prev",
    },
    breakpoints: {
      766: {
        spaceBetween: 10,
...
// end largepanel__product_slider
...
// start related__slider
const relatedsliderfive = document.querySelector(".related__slider_five");
if (relatedsliderfive) {
  var relatedSlider = new Swiper(".related__slider_five", {
    loop: false,
    spaceBetween: 40,
    slidesPerView: 4,
    touchRatio: 0.2,
    ...
    speed: 1000,
    navigation: {
      nextEl: ".related__next_five",
      prevEl: ".related__prev_five",
    },
    breakpoints: {
      10000: {
        slidesPerView: 4,
...
if (document.querySelector(".related__item_swiper")) {
  document.querySelectorAll(".related__item_swiper").forEach((n) => {
    const slider = new Swiper(n.querySelector(".related__item_slider"), {
      loop: false,
      slidesPerView: 1,
      speed: 500,
      spaceBetween: 20,
      ...
      allowTouchMove: false,
      navigation: {
        nextEl: n.querySelector(".related__item_slider_next"),
        prevEl: n.querySelector(".related__item_slider_prev"),
      },
    });
  });
}
  ...
  function init() {
    ...
    var data = {
      points: [
        {
          infoPoint:
            '<div id="mapmoscow" class="map__point{% if properties.active %} map__active{% endif %}">\
        ...
        {
          infoPoint:
            '<div id="mapmo" class="map__point{% if properties.active %} map__active{% endif %}">\
        <span class="map__icon"><svg width="21" height="26" viewBox="0 0 21 26" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.9289 13.429C11.6164 13.429 12.205 13.1842 12.6946 12.6946C13.1841 12.205 13.4289 11.6165 13.4289 10.929C13.4289 10.2415 13.1841 9.65291 12.6946 9.16333C12.205 8.67375 11.6164 8.42895 10.9289 8.42895C10.2414 8.42895 9.65289 8.67375 9.16331 9.16333C8.67373 9.65291 8.42893 10.2415 8.42893 10.929C8.42893 11.6165 8.67373 12.205 9.16331 12.6946C9.65289 13.1842 10.2414 13.429 10.9289 13.429ZM10.9289 22.6165C13.4706 20.2831 15.356 18.1633 16.5852 16.2571C17.8144 14.3508 18.4289 12.6581 18.4289 11.179C18.4289 8.90812 17.705 7.04875 16.2571 5.60083C14.8091 4.15291 13.0331 3.42895 10.9289 3.42895C8.82477 3.42895 7.04873 4.15291 5.60081 5.60083C4.15289 7.04875 3.42893 8.90812 3.42893 11.179C3.42893 12.6581 4.04352 14.3508 5.27268 16.2571C6.50185 18.1633 8.38727 20.2831 10.9289 22.6165ZM10.9289 25.929C7.57477 23.0748 5.06956 20.4237 3.41331 17.9758C1.75706 15.5279 0.928934 13.2623 0.928934 11.179C0.928934 8.05395 1.93414 5.56437 3.94456 3.71021C5.95498 1.85604 8.2831 0.928955 10.9289 0.928955C13.5748 0.928955 15.9029 1.85604 17.9133 3.71021C19.9237 5.56437 20.9289 8.05395 20.9289 11.179C20.9289 13.2623 20.1008 15.5279 18.4446 17.9758C16.7883 20.4237 14.2831 23.0748 10.9289 25.929Z" fill="currentColor"/></svg></span>\
          <div class="map__point_block">\
...
let expand = document.querySelectorAll(
  ".equipment__description .equipment__expand_btn"
);
...
if (newtabs) {
  ...
  for (i = 0; i < tnf.length; i++) {
    tnf[i].onclick = function (e) {
      ...
      if (tnfb.classList.contains("hidden")) {
        this.parentElement.nextElementSibling.children[0].classList.remove(
          "active"
        );
  ...
  for (i = 0; i < tns.length; i++) {
    tns[i].onclick = function (e) {
      ...
      var tnsb =
        this.parentElement.parentElement.nextElementSibling.children[1];
      if (tnsb && tnsb.classList.contains("hidden")) {
...
if (tabs) {
  $(".js-tabs .tabs__navigation-link").on("click", function (e) {
    ...
    // orders active
    thisDataAttr === "orders"
      ? profileSection.addClass("orders-tab")
      : profileSection.removeClass("orders-tab");
  });
}

/*** Change avatar ***/
if (document.querySelector(".js-change-avatar")) {
  ...
  // Delete img
  $(".js-delete-img").on("click", function (e) {
    $.ajax({
      type: "post",
      url: "/ajax/avatar.php",
      cache: false,
      dataType: "json",
      data: "id=" + e.target.id + "&del=y",
      success: function (data) {
        $(`img#PIC${e.target.id}`).attr("src", data);
      },
    });
  ...
  $(".js-change-img").on("click", function () {
    ...
    $(`.user-avatar__field#PIC${id}`).click();
  });
  ...
  $(".user-avatar__field").on("change", function () {
    $(`#change-user-avatar${$(this).attr("id")}`).submit();
  });
}

/*** Order filter ***/
if (document.querySelector(".js-order-filter .orders-heading__filters-item")) {
...
const visibilityPasswordSwitcher = document.querySelector(".js-toggle-show-password");
...
const controlInput = document.querySelector(".js-control-submit");

if (controlInput) {
  $(".js-control-submit").on("input", function () {
    const thisParentForm = $(this).closest("form");
    const thisValLength = $(this).val().length;

    thisValLength < 4
      ? $(".from__submit", thisParentForm).addClass("btn--disabled")
      : $(".from__submit", thisParentForm).removeClass("btn--disabled");
  });
}

/*** Registration ***/
const privacyCheck = document.querySelector(".js-toggle-submit-disabled");

if (privacyCheck) {
  $(".js-toggle-submit-disabled").on("click", function () {
...
document.addEventListener(
  "click",
  removeElem("registration__neworg", "data-del", "delete")
);
...
if (selectbox) {
  ...
  class Select {
    ...
    applyListener() {
      document.querySelector("*").addEventListener("click", (e) => {
        const element =
          this.select.contains(e.target) && e.target.closest(SELECT_ACTION);
        ...
        if (this.isCallSelectItemElement(element)) {
          ...
          document
            ...
            .setAttribute(
              "data-select-item",
              element.getAttribute("data-select-item")
            );
        }

        if (
          this.isCallSelectElement(element) !== true &&
          this.selectOverlayIsClickedElement(element) !== true
        ) {
          this.closeSelectList();
        }
      });
  ...
  [...cartuserlogo].forEach(function (li) {
    for (let [index, elem] of [...li.children].entries()) {
      elem.setAttribute("data-select-item", index + 1);
    }
  });
...
if (cartuser) {
  ...
  [...cartuls].forEach(function (li) {
    for (let [index, elem] of [...li.children].entries()) {
      elem.setAttribute("data-select-item", index + 1);
    }
  });

  const cartuserAllList = document.querySelectorAll(
    ".cart__user .cart__user_list"
  );
  const cartuserlogoIMG = document.querySelectorAll(".cart__user_logo img");

  const selectid = function (obselect) {
    const SELECT_TITLE_COUNT = document
      .querySelector("[data-select-title]")
      .getAttribute("data-select-item");
    cartuserAllList.forEach((n) => n.classList.remove("active"));
    cartuserlogoIMG.forEach((n) => n.classList.remove("active"));
    document
      .querySelector(
        '.cart__user_list[data-select-item="' + SELECT_TITLE_COUNT + '"]'
      )
      .classList.add("active");
    document
      .querySelector(
        '.cart__user_logo img[data-select-item="' + SELECT_TITLE_COUNT + '"]'
      )
      .classList.add("active");
  };
...
if (orgshf) {
  ...
  [...userifs].forEach(function (li) {
    for (let [index, elem] of [...li.children].entries()) {
      elem.setAttribute("data-select-item", index + 1);
    }
  });
  [...orgshfi].forEach(function (li) {
    for (let [index, elem] of [...li.children].entries()) {
      elem.setAttribute("data-select-item", index + 1);
    }
  });
    ...
    for (i = 0; i < orgshfi.length; i++) {
      orgshfi[i].onclick = function (e) {
        if (!this.classList.contains("active")) {
          ...
          document
            .querySelectorAll(".user_info_form")
            .forEach((n) => n.classList.remove("active"));
          this.classList.add("active");
          document
            .querySelector(
              '.user_info_form[data-select-item="' + orgshfiCOUNT + '"]'
            )
            .classList.add("active");
        }
      };
  ...
  function removeElemForm(delElemForm, attributeForm, attributeNameForm) {
    ...
    return function (e) {
      let target = e.target;
      if (
        !(target.hasAttribute(attributeForm)
          ? target.getAttribute(attributeForm) === attributeNameForm
            ? true
            : false
          : false)
      )
      ...
      while (target != this) {
        if (target.classList.contains(delElemForm)) {
          ...
          document
            .querySelector(
              '.orgs-heading__filters-item[data-select-item="' +
                (userinfoformDelCount - 1) +
                '"]'
            )
            .classList.add("active");
          document
            .querySelector(
              '.user_info_form[data-select-item="' +
                (userinfoformDelCount - 1) +
                '"]'
            )
            .classList.add("active");
  ...
  document.addEventListener(
    "click",
    removeElemForm("user_info_form", "data-form-del", "delete")
  );
}
// end
...
const form_changephoneecode = document.querySelector(
  ".changephonecode__success"
);
const form_changephonecodeforms = document.querySelector(
  ".changephonecode__forms"
);
const formfieldset = document.querySelector(".form__fieldset");
const form_changephonecodebclose = document.querySelector(
...
// start form__group_check
const checkboxinputorgcheck = [...document.querySelectorAll('.checkbox__input_orgcheck')];
const formgroupcheckblock = document.querySelector(".form__group_check_block");
if (formgroupcheckblock) {
  checkboxinputorgcheck.forEach(input => input.addEventListener('input', function(event) {
    if (event.target.checked) {
      formgroupcheckblock.classList.add('active');
    } else {
      formgroupcheckblock.classList.remove('active');
    }
  }))
}
// end form__group_check
...
// start button more
var relatedbuttonmore = document.querySelector('.related__button_more');
if (relatedbuttonmore) {
  const rwbItems = [...document.querySelectorAll('.related_with_button .related__item')];
  rwbItems.forEach(n => n.classList.add('hidden'));
  
  const rwbItemss = [...document.querySelectorAll('.related__padding .related_with_button')];
  rwbItemss.forEach(n => {
    if(n.children[0]){n.children[0].classList.remove('hidden')};
    if(n.children[1]){n.children[1].classList.remove('hidden')};
    if(n.children[2]){n.children[2].classList.remove('hidden')};
    if(n.children[3]){n.children[3].classList.remove('hidden')};
  });
  
  var rbm = document.querySelectorAll('.related__button_more');
  for (var i = 0; i < rbm.length; i++) {
    rbm[i].addEventListener('click', function() {
      const thiss = this;
      let showPerClick = 4;
      var rwbItemsHidden = thiss.parentNode.querySelectorAll('.related__item.hidden');
      if (rwbItemsHidden.length < showPerClick) {
        showPerClick = rwbItemsHidden.length;
      }
      if (rwbItemsHidden.length === 0) {
        thiss.classList.remove('active');
        showPerClick = 4;
      }
      var rwbItem = thiss.parentNode.querySelectorAll('.related__item');
      for (var t = 0; t < showPerClick; t++) {
        if (rwbItemsHidden.length === 0) {
          for (var n = 0; n < rwbItem.length; n++) {
            rwbItem[n].classList.add('hidden');
            if(rwbItem[0]){rwbItem[0].classList.remove('hidden');}
            if(rwbItem[1]){rwbItem[1].classList.remove('hidden');}
            if(rwbItem[2]){rwbItem[2].classList.remove('hidden');}
            if(rwbItem[3]){rwbItem[3].classList.remove('hidden');}
          }
        } else {
          thiss.classList.remove('active');
          rwbItemsHidden[t].classList.remove('hidden');
          if (rwbItemsHidden.length === showPerClick) {
...
if(document.querySelector('.vacancy__button')) {
  ...
  for (i = 0; i < vacanciesitem.length; i++) {
    vacanciesitem[i].onclick = function(e) {
      const vacanciesitemNext = this.children[1];
      const vacanciesitembottom = document.getElementsByClassName("vacancy__info");
